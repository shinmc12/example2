...
>>1. 1 대 N 통신
>>2. 멀티 프로세싱 서버에서는 한 클라이언트와의 연결을 자식 프로세스를 생성하여 처리했으나 이 단계에서는 한 프로세스 내에 스레드를 생성하여 처리
>>3. 내부 구현은 다중 스레드 방식으로 구현
>>
>> - 추가 설명
>><br>1. 클라이언트로부터 연결 요청이 오면 해당 요청을 맡아 처리할 새 스레드를 생성
...
>><br>2. 연결 요청 시 마다 로그파일 작성하여 클라이언트의 시간, 아이피를 남기도록 함
>><br>3. 업로드, 다운로드 요청 시 마다 파일명과 시간을 남기도록 함
>>
>> FTP – 파일 송수신 처리
